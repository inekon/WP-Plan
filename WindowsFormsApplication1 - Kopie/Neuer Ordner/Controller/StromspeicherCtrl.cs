...
namespace WindowsFormsApplication1
{
    public class StromspeicherCtrl : StromspeicherModel
    {
        ...
        public void ReadSingle(int ID)
        {
            ...
            if (DBReader.HasRows)
            {
                if (!DBReader.IsDBNull(0)) stromspeichermodel.m_ID = (int)DBReader.GetValue(0);
                if (!DBReader.IsDBNull(1)) stromspeichermodel.m_szBezeichner = (string)DBReader.GetString(1);
                if (!DBReader.IsDBNull(2)) stromspeichermodel.m_szTyp = (string)DBReader.GetString(2);
                if (!DBReader.IsDBNull(3)) stromspeichermodel.m_Leistung = (int)DBReader.GetValue(3);
                if (!DBReader.IsDBNull(4)) stromspeichermodel.m_Energie = (int)DBReader.GetValue(4);
                if (!DBReader.IsDBNull(5)) stromspeichermodel.m_Degradation = (double)DBReader.GetValue(5);
                if (!DBReader.IsDBNull(6)) stromspeichermodel.m_Ladezustand = (int)DBReader.GetValue(6); ;
                rows = 1;
            }
            DBReader.Dispose();
            DBReader.Close();
