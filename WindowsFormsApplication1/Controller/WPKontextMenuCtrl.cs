    ...
    class WPKontextMenuCtrl : Form
    {
        ...
        private void ContextMenuItemBearbeiten_Click(object sender, EventArgs e)
        {
            ...
            if (indexes.Count > 0)
            {
                ...
                werzctrl.items[0].Firma = wpctrl.items[0].Firma;
                werzctrl.items[0].Typ = wpctrl.items[0].Typ;
                werzctrl.items[0].ID_Type =  werzctrl.items[0].ID_Type;
                list.Add(werzctrl.items[0]);
                frm_wpitem.m_werzitemlist = list;
                frm_wpitem.SetControls(0);
                frm_wpitem.ShowDialog();
                ...
                list[0].Baujahr = frm_wpitem.item.Baujahr;
                list[0].Regelung = frm_wpitem.item.Leistungsstufen;
                list[0].Typ = frm_wpitem.item.Typ;
                list[0].Firma = frm_wpitem.item.Firma;
                list[0].Modulkosten = frm_wpitem.item.Modulkosten;
                list[0].Nennleistung = frm_wpitem.item.Nennleistung;
                list[0].ID_Type = frm_wpitem.item.ID_Type;

                frm_wpitem.m_werzitemlist = list;

                WizardCtrl wizctrl = new WizardCtrl();

                wizctrl.Del_Projekt_Waermeerzeuger(m_ID_Projekt, list[0].ID);
