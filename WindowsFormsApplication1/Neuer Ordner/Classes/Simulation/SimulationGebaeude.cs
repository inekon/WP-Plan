...
namespace WindowsFormsApplication1
{
    public class SimulationGebaeude
    {
        ...
        public float Max_Waermebedarf = 0;
        public float Gesamter_Waermebedarf = 0;
        ...
        CSExeCOMServer.SimpleObject com = new CSExeCOMServer.SimpleObject();
...
*/
            for (int i = 0; i < ctrl.rows; i++)
            {
                ...
                TagesVerteilung = DBTagesVeteilung(ctrl.Typ);
            ...
            for(int n=0; n < waectrl.rows; n++) 
            {
                ...
                tool.OpenText(szPath);
                
                //Waermebedarf_Extern[i] += tool.textList
            ...
            //Netzverluste 
            Gesamter_Waermebedarf = com.I_vector_summe(Waermebedarf) ; 
            float stundl_netzverluste = 0;
            if (Netzverluste_Einheit == "%")
            {
                stundl_netzverluste = (Gesamter_Waermebedarf * 1000 * Netzverluste) / 876000;
            }
            ...
            Max_Waermebedarf = Maximaler_Waermebedarf(Waermebedarf);

            com.I_normieren(Waermebedarf_sortiert, Max_Waermebedarf);
        ...
        private float[] DBTagesVeteilung(string TagV_Type)
        {
        ...
        {
            float Max_Waermebedarf;
        
            Max_Waermebedarf = 0;
            for(int i = 0; i<8760; i++)
            {
                if(Max_Waermebedarf < Waermebedarf[i]) Max_Waermebedarf = Waermebedarf[i];
            }
   
            return Max_Waermebedarf;
        }
        ...
        public void SimulationErgebis_in_DB()
        {
            ...
            string sql = "update Tab_Simulation_Ergebnis set Max_Waermebedarf=" + 
                Max_Waermebedarf.ToString(CultureInfo.CreateSpecificCulture("en-US")) +
                ", Gesamt_Waermebedarf=" + 
                Gesamter_Waermebedarf.ToString(CultureInfo.CreateSpecificCulture("en-US"));
        ...
        public Ergebnis SimulationErgebis_aus_DB()
        {
            ...
            if (rs.Next())
            {
                result.Max_Waermebedarf = (double)rs.Read("Max_Waermebedarf");
                result.Gesamt_Waermebedarf = (double)rs.Read("Gesamt_Waermebedarf");
            }
