    ...
    class WPCtrl : WPModel
    {
        ...
        public bool Insert()
        {
            ...
            try
            {
                ...
                DBReader.Close();
                DBCommand.CommandText = "INSERT INTO TAB_WP ( ID_WP, WPName, Beschreibung, Typ, Baujahr, Aufstellung," +
                "Nennleistung, maxPTherm, Heizung, Regelung, Modulkosten )" +
                "SELECT " + ID.ToString() + " AS Ausdr1, '" + WPName + "' AS Ausdr2, '" + Beschreibung + "' AS Ausdr3, '" +
                ...
                Regelung + "' AS Ausdr10, " + Modulkosten.ToString() + " AS Ausdr11";
                DBCommand.ExecuteNonQuery();
            }
            catch (OdbcException sqlEx)
            {
